...
class LocalLatentsTransformer(eqx.Module):
    ...
    def __call__(self, batch: DenoiserBatch) -> DenoiserOutput:
        ...
        if self.use_concat and self.concat_features is not None:
            ...
            n_concat = concat_feats.shape[1]
            if n_concat > 0:
                seqs = jnp.concatenate([seqs, concat_feats], axis=1)
                mask_f = jnp.concatenate([mask_f, concat_mask.astype(jnp.float32)], axis=1)
                c = jnp.concatenate([c, jnp.zeros((seqs.shape[0], n_concat, c.shape[-1]))], axis=1)
        ...
        if n_concat > 0 and self.use_advanced_pair and self.concat_pair_linear is not None:
            ...
            b, n_ext, d = seqs.shape[0], seqs.shape[1], pair_rep.shape[-1]
            pair_rep = jnp.concatenate([pair_rep, jnp.zeros((b, n_concat, n_orig, d))], axis=1)
            pair_rep = jnp.concatenate([pair_rep, jnp.zeros((b, n_ext, n_concat, d))], axis=2)
    ...
    def _pairwise_bb_dists(self, src, tgt_coords, has_cb, d_bins):
        ...
        Args:
            src: [b, n_src, 1, 3] — source coords (unsqueezed for broadcasting)
            tgt_coords: [b, n_tgt, 37, 3] — target atom37 coords
            has_cb: [b, n_tgt] — whether each target residue has CB
            d_bins: 1-D bin edges
        Returns:
            [b, n_src, n_tgt, 4*(n_bins+1)] — binned distances to N, CA, C, CB
        """
        # src: [b, n_src, 1, 3] -> [b, n_src, 1, 1, 3] to broadcast with [b, 1, n_tgt, 4, 3]
        # result: [b, n_src, n_tgt, 4]
        ...
        # Bin each atom channel and concatenate: [b, n_src, n_tgt, 4*(n_bins+1)]
        binned = bin_and_one_hot(dists, d_bins)  # [b, n_src, n_tgt, 4, n_bins+1]
        return binned.reshape(*binned.shape[:3], -1)
    ...
    def _extend_pair(self, batch, pair_rep, n_orig, n_concat):
        ...
        has_cb_target = tgt.atom_mask[:, :, 3]  # [b, n_concat]

        # Upper-right block: binder→target distances
        binder_ca = batch.x_t.bb_ca[:, :, None, :]  # [b, n_orig, 1, 3]
        ur_dists = self._pairwise_bb_dists(binder_ca, tgt.coords, has_cb_target, d_bins)
        # Cross-sequence seq_sep is zero (different chains have no meaningful sequence separation)
        ur_sep = jnp.zeros((b, n_orig, n_concat, 127))
        # Hotspot: binder(False) OR target(from batch) — bool OR, not float addition
        h_binder = jnp.zeros((b, n_orig), dtype=jnp.bool_)
        h_target_bool = tgt.hotspot_mask if tgt.hotspot_mask is not None else jnp.zeros((b, n_concat), dtype=jnp.bool_)
        ur_hotspot = (h_binder[:, :, None] | h_target_bool[:, None, :]).astype(jnp.float32)[..., None]
        ur_raw = jnp.concatenate([ur_sep, ur_dists, jnp.ones((b, n_orig, n_concat, 1)), ur_hotspot], axis=-1)
        ur_proj = self.concat_pair_ln(self.concat_pair_linear(ur_raw))

        # Lower-right block: target→target distances
        t_ca = tgt.coords[:, :, 1, :][:, :, None, :]  # [b, n_concat, 1, 3]
        lr_dists = self._pairwise_bb_dists(t_ca, tgt.coords, has_cb_target, d_bins)
        lr_sep = jnp.broadcast_to(relative_seq_sep(n_concat, 127)[None], (b, n_concat, n_concat, 127))
        # Bool addition in PyTorch is OR (True+True=True=1.0), not integer (1+1=2)
        h_bool = tgt.hotspot_mask if tgt.hotspot_mask is not None else jnp.zeros((b, n_concat), dtype=jnp.bool_)
        lr_hotspot = (h_bool[:, :, None] | h_bool[:, None, :]).astype(jnp.float32)[..., None]
        lr_raw = jnp.concatenate([lr_sep, lr_dists, jnp.zeros((b, n_concat, n_concat, 1)), lr_hotspot], axis=-1)
        lr_proj = self.concat_pair_ln(self.concat_pair_linear(lr_raw))

        ext = jnp.zeros((b, n_ext, n_ext, pair_dim))
        ext = ext.at[:, :n_orig, :n_orig].set(pair_rep)
        ext = ext.at[:, :n_orig, n_orig:].set(ur_proj)
        ext = ext.at[:, n_orig:, :n_orig].set(jnp.swapaxes(ur_proj, 1, 2))
